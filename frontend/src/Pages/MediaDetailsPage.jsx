...
// Importação de ícones e Contexto
...
const MediaDetailsPage = () => {
  ...
  // Novos estados para trailers, similares e créditos
  ...
  const {
    isMovieInWatchlist, toggleWatchlist,
    isMovieInFavorites, toggleFavorites, isLoggedIn
  } = useUserLists();

  // Calcula o estado de cada lista
  ...
  const loadMovieData = async (movieId) => {
    try {
      ...
      const [movieData, reviewsData, videosData, similarsData, creditsData] = await Promise.all([
        movieService.getMovieDetails(movieId),
        reviewService.getMovieReviews(movieId).catch(() => []),
  ...
  // Handler para o botão ADICIONAR À LISTA (Watchlist)
  ...
  // Handler para o botão CURTIR (Favorites)
  ...
  // Pega o trailer oficial (YouTube)
      ...
      <div className="bg-[#F4F6F8] rounded-3xl shadow-lg p-6 sm:p-8 md:p-10">
        ...
        >
          ...
          <div className="absolute top-4 right-4 flex flex-col gap-3">
              {/* Botão ADICIONAR À LISTA (Watchlist) */}
              <button onClick={handleToggleWatchlist} className={`text-white hover:opacity-80 backdrop-blur-sm transition-all duration-300 transform font-semibold cursor-pointer z-10 px-4 py-2 rounded-full flex items-center gap-2
                  ${
                      isInWatchlist
                      ? 'bg-[#00B5AD] scale-105'
              ...
              {/* NOVO BOTÃO: CURTIR (Favorites) - Animação e Lógica de Favoritos */}
              <button onClick={handleToggleFavorites} className={`text-white hover:opacity-80 backdrop-blur-sm transition-all duration-200 transform font-semibold cursor-pointer z-10 px-4 py-2 rounded-full flex items-center gap-2
                  ${
                      isInFavorites
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-10">
          ...
          <div>
            ...
            <div className="space-y-4">
              {credits && credits.cast && credits.cast.slice(0, showAllCast ? credits.cast.length : 5).map((actor) => (
              ...
              {credits && credits.cast && credits.cast.length > 5 && (
                ...
                >
                  {showAllCast ? 'Ver Menos' : `Ver Mais (${credits.cast.length - 5})`}
                </button>
              )}
            </div>
          </div>
        ...
        {/* FILMES SIMILARES SECTION */}
        {similarMovies && similarMovies.length > 0 && (
          <div className="mt-10">
            <h2 className="text-3xl font-bold text-[#2D3748] mb-6">Filmes Similares</h2>
              ...
              {similarMovies.map((similarMovie) => (
                <MovieCard
                  ...
                  movie={{
                    ...
                    release_date: similarMovie.release_date || new Date().toISOString()
                  }}
                />
              ))}
            </div>
